...
import { PARENT_HREF } from "src/constants/navbarConstant";
...
function ParentSideBarItem({ listClassroom }) {
...
function ClassroomItem({ item }) {
  return (
    <div className="flex items-center gap-2 py-1">
      <Link
        to={`/parent-classrooms/${item?.classId}`}
      ...
      <Link
        ...
        to={`/assessments?id=${item?.classId}`}
      >
        Assessments
      </Link>
    </div>
